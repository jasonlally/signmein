 ...
 *
 * $nav['about/history'] = array('label' => 'About', 'parent_id' =>
 * 'about');
 * OR
 * $nav['about/history'] = array('location' => 'about_us/history',
 * label' => 'About', 'parent_id' => 'about');
...
class Menu
{
	...
	// the css class for the first menu item
	public $last_class = 'last';
	// the css class for the last menu item
	public $depth = NULL;
	// the depth of the menu to render at
	public $use_titles = TRUE;
	// use the title attribute in the links
	public $root_value = NULL;
	// the root parent value... can be NULL or 0
	public $container_tag = 'ul';
	// the html tag for the container of a set of menu items
	public $container_tag_attrs = '';
	// html attributes for the container tag
	public $container_tag_id = '';
	// html container id
	public $container_tag_class = '';
	// html container class
	public $cascade_selected = TRUE;
	// cascade the selected items
	public $include_hidden = FALSE;
	// include menu items with the hidden attribute
	public $item_tag = 'li';
	// the html list item element
	public $item_id_prefix = '';
	// the prefix to the item id
	public $item_id_key = 'id';
	// either id or location
	public $use_nav_key = 'AUTO';
	// use the nav_key value to match active
	public $render_type = 'basic';
	// basic, breadcrumb, page_title, collapsible, delimited, array
	public $pre_render_func = '';
	// function to apply to menu labels before rendering
	public $logged_in = FALSE;

	// for breadcrumb AND/OR page_title
	public $delimiter = FALSE;
	// the html element between the links
	public $display_current = TRUE;
	...
	// the class for the arrows

	// for page_title ONLY
	public $order = 'asc';
	// the order to display... for page_title ONLY

	protected $_items = array();
	// the items in the menu
	protected $_active_items = array();
	// the active menu items
	protected $_reset_params = array();
	// reset params
	...
	public function normalize_items($items)
	{
		...
		if (is_array($items))
		{
			...
			foreach ($items as $key => $val)
			{
				...
				// check to make sure parent_id does not equal id to prevent infinite
				// loops
	 ...
	 * @param	mixed int or string of the parent id to begin rendering the
	 * menu items
	...
	protected function _render_basic($menu, $level = -1)
	{
		...
		{
			// filter out hidden ones first. Need to do in seperate loop in case
			// there is a hidden one at the end
			...
			if ( ! empty($menu))
			{
				...
				foreach ($menu as $key => $val)
				{
					...
					{
						$i = - 1;
						//if there is a header in this menu, don't count it as an item
						$last_menu_count = count($menu) - 2;
					} else
					{
						$i = 0;
					}
					$str .= $this->_create_open_li($val, $level, $i, ($i == $last_menu_count));
	...
	{
		// filter out hidden ones first. Need to do in seperate loop in case
		// there is a hidden on e at the end
		$menu = $this->_filter_hidden($menu);
	...
	protected function _get_li_classes($val, $level, $i, $is_last = FALSE)
	{
		...
		if ($val['header'])
			$css_classes[] = 'nav-header';

		if ($this->active == $active OR ($this->cascade_selected AND is_array($this->_active_items) AND in_array($active, $this->_active_items)))
		{
	...
	protected function _get_menu_items($parent, $items = NULL)
	{
		...
		foreach ($items as $key => $val)
		{
			// force numbers to be integers if numeric so we can do a strict
			// comparison
